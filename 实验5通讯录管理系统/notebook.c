...
void insertYouXu(LinkList L,LinkList Elem)//插入一个元素，使原有序表仍为有序 
{
	LinkList p=L->next;
...
LinkList creatlncreLink()//创建一个存放通讯录成员的非递减有序表，返回头结点地址 
{
	...
	char name[20];//姓名
	double telenum;//电话号码
	printf("请输入学生学号、姓名和电话号码，建立通讯录，结束标志为'-1'\n");
	printf("学号 #%d:",num);
	scanf("%d",&number);
	printf("姓名 #%d: ",num);
	char temp=getchar();
...
int searchNum(LinkList L,int n)//按学号查找通讯者 
{
	int flag=0;
	LinkList p=L->next;
	seat=1;
...
int searchName(LinkList L,char n[])//按姓名查找通讯者 
{
	int flag=0;
	LinkList p=L->next;
	seat=1;
...
int delNum(LinkList L,int n)//按学号删除通讯者 
{
	int flag=0;
	LinkList p=L->next;
	seat=1;
	if(L->next==NULL)
		printf("该通讯录中没有成员记录,删除失败\n");
	else{
		while(p!=NULL){			
			if(p->number<=n){
				if(p->number==n){
...
int delName(LinkList L,char n[])//按姓名删除通讯者 
{
	int flag=0;
	LinkList p=L->next;
	seat=1;
